...
(() => {
  ...
  const mount = () => {
    ...
    const summaryNoteSleep = sleepHours ? sleepQuality : 'Registre sua primeira noite';
    const summaryNoteRoutine = routineTotal ? labelTime() + ' • ' + tasksDone + ' tarefas conclu\xeddas' : 'Crie sua primeira rotina';
    const summaryNoteFinance = nextAgenda ? 'Pr\xF3ximo: ' + (nextAgenda.title || nextAgenda.name || 'compromisso') : 'Nenhum compromisso pr\xF3ximo';

    const card = document.createElement('section');
    card.className = 'home-personal-summary polished-card';
    card.innerHTML =
      '<h2>RESUMO PESSOAL</h2>' +
      '<div class="summary-subtitle">Seu espa\xE7o em um olhar, sem excesso de informa\xE7\xE3o.</div>' +
      '<div class="summary-grid">' +
      '<div class="summary-row"><div class="summary-label">Sono</div><div class="summary-value">' + (sleepHours ? sleepHours.toFixed(1) + 'h' : 'Sem registro') + '</div><div class="summary-note">' + summaryNoteSleep + '</div></div>' +
      '<div class="summary-row"><div class="summary-label">Rotina</div><div class="summary-value">' + routineDone + '/' + (routineTotal || 0) + ' conclu\xEDdos</div><div class="summary-note">' + summaryNoteRoutine + '</div></div>' +
      '<div class="summary-row"><div class="summary-label">Finan\xE7as</div><div class="summary-value">' + money(financeBalance) + '</div><div class="summary-note">' + summaryNoteFinance + '</div></div>' +
      '</div>' +
...
Write-Host 'home-summary-encoding-fixed'
