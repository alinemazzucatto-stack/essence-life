...
import { isProtectedAppPath } from './shared/route-access';
...
const unregisterHomeWater=()=>{let entries:{id:string;date:string;time:string;ml:number}[]=[];try{const saved=JSON.parse(localStorage.getItem('essence:water-entries')||'[]');if(Array.isArray(saved))entries=saved}catch{}const index=entries.findIndex(item=>item.date===today());if(index<0)return water;const removed=entries[index];if(removed.id===localStorage.getItem('essence:water-home-check:'+today()))localStorage.removeItem('essence:water-home-check:'+today());const nextEntries=entries.filter((_,entryIndex)=>entryIndex!==index);localStorage.setItem('essence:water-entries',JSON.stringify(nextEntries));const nextTotal=nextEntries.filter(item=>item.date===today()).reduce((sum,item)=>sum+(Number(item.ml)||0),0);setWater(nextTotal);localStorage.setItem('essence:water',String(nextTotal));return nextTotal};const toggleHomeWaterCheck=()=>{const checkKey='essence:water-home-check:'+today();let entries:{id:string;date:string;time:string;ml:number}[]=[];try{const saved=JSON.parse(localStorage.getItem('essence:water-entries')||'[]');if(Array.isArray(saved))entries=saved}catch{}const savedId=localStorage.getItem(checkKey)||'';const current=entries.find(entry=>entry.id===savedId&&entry.date===today());let nextEntries=entries,checked=false;if(current){nextEntries=entries.filter(entry=>entry.id!==current.id);localStorage.removeItem(checkKey)}else{const entry={id:id(),date:today(),time:new Date().toTimeString().slice(0,5),ml:250};nextEntries=[entry,...entries];localStorage.setItem(checkKey,entry.id);checked=true}localStorage.setItem('essence:water-entries',JSON.stringify(nextEntries));const total=nextEntries.filter(entry=>entry.date===today()).reduce((sum,entry)=>sum+(Number(entry.ml)||0),0);setWater(total);localStorage.setItem('essence:water',String(total));return {total,checked}};return <AuthenticatedLayout session={session} page={page} mobileNavOpen={mobileNavOpen} onToggleMobileNav={()=>setMobileNavOpen(value=>!value)} openPage={openPage} isLocked={isLocked} showPlanLocks={!(developerMode||localHost)} onLogout={()=>{void signOut().finally(()=>{localStorage.removeItem('essence:session');setSession(null);window.location.assign('/')})}}>{lockedFeature?<AccessGate title={pageMeta[lockedFeature.page].title} required={gateRequired} onOpenPlans={openProfilePlans}/>:<>{page==='home'&&<Home session={session} pendingHabits={pendingHabits} pendingTasks={pendingTasks} completedHabits={completedHabits} completedTasks={completedTasks} totalToday={totalToday} completedToday={completedToday} dayProgress={dayProgress} waterToday={water} quickCreate={quickCreate} setQuickCreate={setQuickCreate} openPage={openPage} registerHomeWater={registerHomeWater} unregisterHomeWater={unregisterHomeWater} toggleHomeWaterCheck={toggleHomeWaterCheck} completeHomeHabit={completeHomeHabit} completeHomeTask={completeHomeTask} setHomeTasks={setHomeTasks} setHomeHabits={setHomeHabits}/>}{page==='agenda'&&<Agenda/>}{page==='pomodoro'&&<Pomodoro/>}{page==='sleep'&&<SleepModule/>}{page==='workouts'&&<Workouts/>}{page==='beauty'&&<BeautyCare/>}{page==='diary'&&<Diary/>}{page==='insights'&&<Insights openPage={openPage}/>} {page==='settings'&&<Settings onInstall={installApp} canInstall={!!installPrompt} isInstalled={isInstalled}/>}
