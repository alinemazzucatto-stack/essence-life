...
{page==='profile'&&<Profile session={session} onUpdate={setSession} onInstall={installApp} canInstall={!!installPrompt} isInstalled={isInstalled} onOpenSettings={()=>openPage("settings")}/>}{page==='routine'&&<Routine/>}{page==='cycle'&&<Cycle/>}{page==='finance'&&<Finance canAccess={developerMode||localHost||planMeets(subscription.plan,'premium')} onOpenPlans={()=>openPage('profile')}/>}{page==='house'&&<House canAccessFinance={developerMode||localHost||planMeets(subscription.plan,'premium')}/>} {page==='nutrition'&&<Nutrition setWater={setWater}/>}</>}</AuthenticatedLayout>}function App(){const path=window.location.pathname;const appRoute=path==='/app'||path==='/app/';const checkout=path==='/comprar/essencial'||path==='/comprar/pro';return checkout?<Checkout/>:appRoute?<EssenceApp/>:<DiscoveryQuiz/>}
export default App;
