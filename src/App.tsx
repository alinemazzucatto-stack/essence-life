...
const completeHomeTask=(taskId:string)=>setHomeTasks(items=>{const next=items.map(task=>task.id===taskId?{...task,done:true}:task);localStorage.setItem('essence:tasks',JSON.stringify(next));removeAppReminder(taskId);return next});
const completeHomeHabit=(habitId:string)=>setHomeHabits(items=>{const next=items.map(habit=>habit.id===habitId?{...habit,done:true,doneDate:today()}:habit);localStorage.setItem('essence:routine-items',JSON.stringify(next));const history=readStoredArray<{routineId:string;date:string}>('essence:routine-history');if(!history.some((entry:{routineId:string;date:string})=>entry.routineId===habitId&&entry.date===today()))localStorage.setItem('essence:routine-history',JSON.stringify([{id:id(),routineId:habitId,date:today(),completedAt:new Date().toISOString()},...history]));return next});
...
{page==='profile'&&<Profile session={session} onUpdate={setSession} onInstall={installApp} canInstall={!!installPrompt} isInstalled={isInstalled} onOpenSettings={()=>openPage("settings")}/>}{page==='routine'&&<Routine/>}{page==='cycle'&&<Cycle/>}{page==='finance'&&<Finance canAccess={developerMode||localHost||planMeets(subscription.plan,'premium')} onOpenPlans={()=>openPage('profile')}/>}{page==='house'&&<House canAccessFinance={developerMode||localHost||planMeets(subscription.plan,'premium')}/>} {page==='nutrition'&&<Nutrition setWater={setWater}/>}</>}</AuthenticatedLayout>}function App(){const [showLaunch,setShowLaunch]=useState(true);useEffect(()=>{const timer=window.setTimeout(()=>setShowLaunch(false),1150);return()=>window.clearTimeout(timer)},[]);const path=window.location.pathname;const nativeApp=Capacitor.isNativePlatform()||Capacitor.getPlatform()==='android'||Capacitor.getPlatform()==='ios'||window.location.protocol==='capacitor:'||navigator.userAgent.includes('; wv)');const appRoute=nativeApp||path==='/app'||path==='/app/';const checkout=path==='/comprar/essencial'||path==='/comprar/pro';if(showLaunch&&appRoute)return <main className="essence-launch" aria-label="Abrindo Essence Life"><img src="/essence-life-launch.png" alt="Essence Life"/><span>Essence Life</span></main>;return checkout?<Checkout/>:appRoute?<EssenceApp/>:<DiscoveryQuiz/>}
export default App;
