...
useEffect(()=>{let stopSync:(()=>void)|undefined;restoreOnlineSession().then(async online=>{if(!online){setSession(null);setPlanChecked(true);return}const cloudChanged=await restoreCloudState(online).catch(()=>false);if(cloudChanged){window.location.reload();return}setSession(mergeSavedProfile({name:online.user.name,email:online.user.email}));const plan=await readEntitlement(online);setSubscription(current=>({...current,plan}));setPlanChecked(true);stopSync=startCloudSync(online)});return()=>stopSync?.()},[]);
