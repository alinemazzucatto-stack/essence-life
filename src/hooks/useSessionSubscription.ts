...
import { restoreCloudState, startCloudSync } from '../shared/cloud-sync';
...
useEffect(()=>{let stopSync:(()=>void)|undefined;restoreOnlineSession().then(async online=>{if(!online){setSession(null);setPlanChecked(true);return}await restoreCloudState(online).catch(()=>false);setSession(mergeSavedProfile({name:online.user.name,email:online.user.email}));const plan=await readEntitlement(online).catch(()=>subscription.plan);setSubscription(current=>({...current,plan}));setPlanChecked(true);stopSync=startCloudSync(online)}).catch(()=>{setPlanChecked(true)});return()=>stopSync?.()},[]);
